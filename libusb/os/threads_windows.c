...
__inline static int usbi_cond_intwait(usbi_cond_t *cond,
    ...
    if(!found) {
        pos = (struct usbi_cond_perthread*) calloc(1, sizeof(struct
                                                             usbi_cond_perthread));
        if(!pos)
            return ((errno=ENOMEM)); // This errno is not POSIX-allowed.
        pos->tid = tid;
...
// N.B.: usbi_cond_*wait() can also return ENOMEM, even though pthread_cond_*wait
// cannot!
