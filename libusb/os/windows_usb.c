...
struct libusb_device * get_hotplug_device_node( const char* name, struct libusb_context* ctx, int online, uint8_t hcd, bool hotplug_poll )
{
	...
	char *devId, *class, *guid, *dev_interface_path, *dev_id_path;
	SP_DEVINFO_DATA dev_info_data = { 0 };

	dev = parent_dev = NULL;
	devId = guid = dev_interface_path = dev_id_path = NULL;
	...
	// enuemrate by class
	devinfo = pSetupDiGetClassDevsA( NULL, class, NULL, dwFlag);
	...
	for(i = 0; pSetupDiEnumDeviceInfo(devinfo, i, &dev_info_data); i++){
		...
		ret = pSetupDiGetDeviceInstanceIdA(devinfo, &dev_info_data, buf, sizeof(buf), &size);
		if (!ret){
			usbi_err(ctx, "SetupDiGetDeviceInfoInstanceId failed %s", windows_error_str(0));
			goto cleanup;
		}
	...
	if ( ret != CR_SUCCESS ){
		usbi_err(ctx, "CM_GET_DEVICE_IDA failed %s", windows_error_str(0));
		goto cleanup;
	}
	...
	ret = pSetupDiGetDeviceRegistryPropertyA( devinfo, &dev_info_data, SPDRP_ADDRESS, &reg_type, (BYTE *)&port_nr, 4, &size );
	if ( !ret || size!= 4 ){
	...
	ret = pSetupDiGetDeviceRegistryPropertyA(devinfo, &dev_info_data, SPDRP_INSTALL_STATE, &reg_type, (BYTE*)&install_state, sizeof(install_state), &size);
	if(!ret || size!=4){
	...
	if (parent_dev == NULL){
		char* parent_path;
		parent_path = get_ancestor_session_path(dev_info_data.DevInst, 1);
		if (parent_path == NULL){
			usbi_warn(ctx, "Failed to find parent session_path");
			goto cleanup;
		}
		if (hcd==0){
			// connected, not hcd
			parent_dev = get_hotplug_device_node(parent_path, ctx, TRUE, FALSE, hotplug_poll );
			if (parent_dev == NULL) {
	...
	default:
		...
		if (priv->usb_interface[0].path != NULL) {
			...
			usbi_warn(NULL, "could not duplicate interface path '%s'", priv->path);
		}
	...
	ret = init_device(dev, parent_dev, (uint8_t) port_nr, dev_id_path, dev_info_data.DevInst);
...
LRESULT CALLBACK message_callback_handle_device_change(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	list_for_each_entry( ctx, &active_contexts_list, list, struct libusb_context){
...
#ifdef UNICODE
		temp_len = wcslen(dev_bdi->dbcc_name);
		name = (char *) calloc(temp_len, sizeof(char));
		if (wcstombs(name, dev_bdi->dbcc_name, temp_len) != temp_len){
			usbi_err(ctx, "failed converting from wide char to ansi");
			free(name);
			continue;
		}
#else
		name = dev_bdi->dbcc_name;
#endif
		// connected depends on DBT_DEVICEARRIVAL been set, it's not an hcd, this is surely not hotplug_poll
		dev = get_hotplug_device_node(name, ctx, (wParam == DBT_DEVICEARRIVAL), FALSE, FALSE);
		...
		usbi_info(ctx, "(bus: %d, addr: %d, prod: %04X:%04X, state: %s)", dev->bus_number, dev->device_address,
			  dev->device_descriptor.idVendor, dev->device_descriptor.idProduct, online?"INSERTION":"REMOVAL");
 ...
 */
// TODO: Windows limitations mean we cannot detect driverless devices with this method
...
#define HOTPLUG_CLASS "libusb_messaging_class"
#define HOTPLUG_WINDOW "libusb_messaging"
...
unsigned __stdcall windows_hotplug_threaded(void* param)
{
	...
	wc.lpszClassName = HOTPLUG_CLASS;
	...
	hHotplugMessage = CreateWindowExA( 0, HOTPLUG_CLASS, HOTPLUG_WINDOW, 0, 0, 0, 0, 0, HWND_MESSAGE, NULL, NULL, NULL);

	if (hHotplugMessage == NULL) {
		usbi_err(ctx, "Unable to create hotplug window: %s", windows_error_str(0));
		res = LIBUSB_ERROR_ACCESS;
		goto hotplug_unregister;
	}
	...
	notify = RegisterDeviceNotificationA(hHotplugMessage, &dev_bdi, DEVICE_NOTIFY_WINDOW_HANDLE);
	if ( NULL == notify ) {
		res = LIBUSB_ERROR_ACCESS;
		goto hotplug_destroy_window;
	}
	...
	usbi_info(ctx, "message queue completed");

	if (UnregisterDeviceNotification(notify) == 0){
		usbi_err(ctx, "couldn't unregister notification %s", windows_error_str(0));
	}

hotplug_destroy_window:
	if (DestroyWindow(hHotplugMessage) == 0){
		usbi_err(ctx, "failed destroying windows %s", windows_error_str(0));
	}
	hHotplugMessage = NULL;

hotplug_unregister:
	if (UnregisterClassA(HOTPLUG_CLASS, NULL) == 0){
		usbi_err(ctx, "couldn't unregister class %s", windows_error_str(0));
		res = LIBUSB_ERROR_ACCESS;
	}
hotplug_exit:
	usbi_dbg("terminated thread");
...
void windows_hotplug_poll(void){
	...
	list_for_each_entry(ctx, &active_contexts_list, list, struct libusb_context) {
		...
		usbi_dbg("################## HCD pass ####################");
		for(i=0; ; i++){
			dev_interface_details = get_interface_details( ctx, &dev_info, &dev_info_data, &GUID_DEVINTERFACE_USB_HUB, i);

			if (dev_interface_details == NULL){
				break;
			}

			// NODE should be connected, and as it's an hcd it should have i+i
			dev = get_hotplug_device_node( dev_interface_details->DevicePath, ctx, TRUE, i+1, TRUE);
		...
		for(i=0; ; i++){
			dev_interface_details = get_interface_details(ctx, &dev_info, &dev_info_data, &GUID_DEVINTERFACE_USB_DEVICE, i);

			if (dev_interface_details == NULL){
				break;
			}

			dev = get_hotplug_device_node(dev_interface_details->DevicePath, ctx, TRUE, FALSE, TRUE);
